...
>>回到本题，箭头函数的`this`指向的是它的执行上下文，简而言之要么是包含剪头函数的常规函数，要么是全局对象，上述`perimeter`的`this`则是指向的是全局执行上下文`window`
</p>
...
> * 消除魔术字符串，减少耦合
>* 遍历：symbol属性名不能被诸如`for..in、Object.keys()、Object.getOwnPropertyNames()、JSON.stringify()`等遍历到，需要使用 `Object.getOwnPropertySymbols(obj)`，一个新的 API，`Reflect.ownKeys(obj)`方法可以返回所有类型的键名，包括常规键名和 Symbol 键名,该方法与`Object.keys(obj)`类似
>
>[symbol传送门](http://es6.ruanyifeng.com/#docs/symbol)
</p>
...
`next`方法返回一个对象，它的`value`属性值是当前`yield`表达式的值，如上述`"Do you love JavaScript?"`,`done`属性是一个布尔值，表示是否遍历结束。

Generator函数从上次`yield`表达式停下的地方，一直执行到return语句（如果没有return语句，就执行到函数结束)。`next`方法返回的最后一个对象的`value`属性的值就是紧跟在`return`语句后面的表达式的值（如果没有`return`语句，则`value`属性的值为`undefined`），此时`done`属性的值`true`，表示遍历已经结束。
